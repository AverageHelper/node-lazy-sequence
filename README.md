...
A lazy sequence, as implemented here, doesn't have much performance improvement over regular `Array`s until the array contains about 80,000 items. Algorithmically, the benefits are clear. Realistically, computers prefer repetitive tasks, like running lots of `map`s and then lots of `filter`s, to switching between `map` and `filter` operations quickly.
